...
export default function JournalPage() {
  ...
  const [reflection, setReflection] = useState("");
  const [message, setMessage] = useState("");
  const [hasStartedJournal, setHasStartedJournal] = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updated = await getAllEntries(user.id);
      setEntries(updated);
      setHasStartedJournal(true);
      setMessage(`Saved Day ${dayNumber} ✨`);
      setTimeout(() => setMessage(""), 2000);
  ...
  // -------------------------
  // LOGIN VIEW (ONLY ONCE)
  // -------------------------
  if (!user && !hasStartedJournal) {
    return (
      <section
        style={{
          padding: "4rem 1.5rem",
      ...
      >
        ...
        <p style={{ opacity: 0.8 }}>
          Log in above to access your private 21-day compassion journal.
        </p>
      </section>
    );
  }
    ...
    >
      ...
      <form
        ...
        style={{
          ...
          backgroundImage:
            "repeating-linear-gradient(#fffef9, #fffef9 28px, #f3f4f6 29px)",
        }}
      >
        ...
        <textarea
          rows={4}
          placeholder="How did you live with compassion today?"
          value={reflection}
          onChange={(e) => setReflection(e.target.value)}
          style={noteTextarea}
        />
      ...
      <div style={{ maxWidth: "720px", margin: "0 auto" }}>
        <h3 style={{ marginBottom: "1rem" }}>Your journal pages</h3>

        {loading ? (
          <p>Loading…</p>
        ) : entries.length === 0 ? (
          <p style={{ opacity: 0.7 }}>
            Your notebook is empty. Start with Day 1 ✨
          </p>
        ) : (
          entries
            .slice()
            ...
            .map((entry, index) => (
              <div
                ...
                style={{
                  ...
                  marginBottom: "1rem",
                  borderRadius: "14px",
                  background: index === 0 ? "#fff7ed" : "#f8fff9",
